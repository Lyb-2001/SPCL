...
if True:
    ...
    class CrossScan1D_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x1: torch.Tensor, x2: torch.Tensor):
            B,C,L = x1.shape
            ctx.shape = (B, C,L)

            xs_fuse = x1.new_empty((B, 2, C, 2 * L))
            # print(x1.shape)
            # 方向0: 行优先正向扫描
            xs_fuse[:, 0, :, 0::2] = x1  # RGB特征
            xs_fuse[:, 0, :, 1::2] = x2  # 另一模态
            xs_fuse[:, 1] = torch.flip(xs_fuse[:, 0], dims=[-1])
            return xs_fuse  # 输出形状 [B, 2, 1, 2C]
        ...
        def backward(ctx, ys):
            ...
            # 合并四个方向的梯度
            y_row = ys[:, 0] + ys[:, 1].flip(dims=[-1])  # 行方向
            # y_col = ys[:, 1] + ys[:, 3].flip(dims=[-1])  # 列方向

            # 重建原始维度
            grad_rgb = y_row[:, :, 0::2].view(B, -1, H)
            grad_e = y_row[:, :, 1::2].view(B, -1, H)

            return grad_rgb, grad_e


    class CrossMerge1D_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor, H: int):
            ctx.save_for_backward(torch.tensor([H]))
            B, K, C, L = ys.shape

            # 处理行扫描
            y_row = ys[:, 0] + ys[:, 1].flip(dims=[-1])
            rgb_row = y_row[:, :, 0::2].view(B, C, H)
            e_row = y_row[:, :, 1::2].view(B, C, H)



            return rgb_row , e_row

        @staticmethod
        def backward(ctx, grad_rgb, grad_e):
            H = ctx.saved_tensors[0]
            B, C, _ = grad_rgb.shape

            # 重建扫描序列
            xs = grad_rgb.new_empty((B, 2, C, 2 * H))
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        ...
        def forward(ctx, x_rgb: torch.Tensor, x_e: torch.Tensor):
            ...
            # 初始化4个扫描方向（行正向、列正向、行反向、列反向）
            xs_fuse = x_rgb.new_empty((B, 4, C, 2 * H * W))

            # 方向0: 行优先正向扫描
            xs_fuse[:, 0, :, 0::2] = x_rgb.flatten(2, 3)  # RGB特征
            xs_fuse[:, 0, :, 1::2] = x_e.flatten(2, 3)  # 另一模态

            # 方向1: 列优先正向扫描（通过转置实现）
            xs_fuse[:, 1, :, 0::2] = x_rgb.transpose(2, 3).flatten(2, 3)
            xs_fuse[:, 1, :, 1::2] = x_e.transpose(2, 3).flatten(2, 3)

            # 方向2: 行优先反向扫描
            xs_fuse[:, 2] = torch.flip(xs_fuse[:, 0], dims=[-1])

            # 方向3: 列优先反向扫描
            xs_fuse[:, 3] = torch.flip(xs_fuse[:, 1], dims=[-1])
        ...
        def backward(ctx, ys):
            ...
            # 合并四个方向的梯度
            y_row = ys[:, 0] + ys[:, 2].flip(dims=[-1])  # 行方向
            y_col = ys[:, 1] + ys[:, 3].flip(dims=[-1])  # 列方向

            # 重建原始维度
            grad_rgb = y_row[:, :, 0::2].view(B, -1, H, W) + y_col[:, :, 0::2].view(B, -1, W, H).transpose(2, 3)
            grad_e = y_row[:, :, 1::2].view(B, -1, H, W) + y_col[:, :, 1::2].view(B, -1, W, H).transpose(2, 3)
    ...
    class CrossMerge_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor, H: int, W: int):
            ctx.save_for_backward(torch.tensor([H, W]))
            B, K, C, L = ys.shape

            # 处理行扫描
            y_row = ys[:, 0] + ys[:, 2].flip(dims=[-1])
            rgb_row = y_row[:, :, 0::2].view(B, C, H, W)
            e_row = y_row[:, :, 1::2].view(B, C, H, W)

            # 处理列扫描
            y_col = ys[:, 1] + ys[:, 3].flip(dims=[-1])
            rgb_col = y_col[:, :, 0::2].view(B, C, W, H).transpose(2, 3)
            e_col = y_col[:, :, 1::2].view(B, C, W, H).transpose(2, 3)

            return rgb_row + rgb_col, e_row + e_col

        @staticmethod
        def backward(ctx, grad_rgb, grad_e):
            H, W = ctx.saved_tensors[0].tolist()
            B, C, _, _ = grad_rgb.shape

            # 重建扫描序列
            xs = grad_rgb.new_empty((B, 4, C, 2 * H * W))

            # 行正向
            row_rgb = grad_rgb.flatten(2)
            row_e = grad_e.flatten(2)
            xs[:, 0, :, 0::2] = row_rgb
            xs[:, 0, :, 1::2] = row_e

            # 列正向
            col_rgb = grad_rgb.transpose(2, 3).flatten(2)
            col_e = grad_e.transpose(2, 3).flatten(2)
            xs[:, 1, :, 0::2] = col_rgb
            xs[:, 1, :, 1::2] = col_e
    ...
    def cross_selective_scan_1d_multimodal(
            x1: torch.Tensor,
            x2: torch.Tensor,
            x_proj_weight: torch.Tensor,
            x_proj_bias: Optional[torch.Tensor],
            dt_projs_weight: torch.Tensor,
            dt_projs_bias: Optional[torch.Tensor],
            # <<< 变化 1: 增加 dt_rank 参数
            dt_rank: int,
            A_logs: torch.Tensor,
            Ds: torch.Tensor,
            out_norm1: torch.nn.Module,
            out_norm2: torch.nn.Module,
        ...
        """
        修正后的1D多模态交叉选择性扫描函数。
        - x1, x2: 输入的1D特征向量，形状为 [B, L]。
        - ...params: SSM相关的参数。
        - out_norm1, out_norm2: 对两个模态输出的归一化层。
        """
        B, C = x1.shape
        L = 2 * C
        K, D_in = 2, 1
        ...
        x_fuse = CrossScan1D_multimodal.apply(x1, x2)

        x_dbl = torch.einsum("b k d l, k c d -> b k c l", x_fuse, x_proj_weight)
        if x_proj_bias is not None:
            x_dbl += x_proj_bias.view(1, K, -1, 1)

        # <<< 变化 2: 使用正确的 dt_rank 和 d_state 进行分割
        dts, Bs, Cs = torch.split(x_dbl, [dt_rank, d_state, d_state], dim=2)

        dts = torch.einsum("b k r l, k d r -> b k d l", dts, dt_projs_weight)

        x_fuse = x_fuse.view(B, -1, L).float()
        dts = dts.contiguous().view(B, -1, L).float()
        As = -torch.exp(A_logs.float())
        Bs = Bs.contiguous().float()
        Cs = Cs.contiguous().float()
        Ds = Ds.float()
        delta_bias = dt_projs_bias.view(-1).float() if dt_projs_bias is not None else None

        ys = SelectiveScan.apply(x_fuse, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus, 1)
        ys = ys.view(B, K, -1, L)

        y1, y2 = CrossMerge1D_multimodal.apply(ys, C)
        ...
        return y1.to(x1.dtype), y2.to(x2.dtype)
    ...
    class MambaChannelAttention(nn.Module):
        ...
        def __init__(
            ...
            self.proj_avg = nn.Linear(self.d_model, self.d_inner*2, bias=bias, **factory_kwargs)
            self.proj_max = nn.Linear(self.d_model, self.d_inner*2, bias=bias, **factory_kwargs)
            self.avg_pool = nn.AdaptiveAvgPool2d(1)
            self.linearout = nn.Linear(self.d_inner,1)
            self.linearout1 = nn.Linear(self.d_inner,1)
        ...
        def forward_corev2_multimodal(self, x1, x2, nrows=1):
            return cross_selective_scan_multimodal_1d(
        ...
        def forward(self, avg_feat: torch.Tensor, max_feat: torch.Tensor):
            # print(self.d_model)
            xx1 = self.proj_avg(avg_feat)
            xx2 = self.proj_max(max_feat)
            x1, z1 = xx1.chunk(2, dim=-1)
            x2, z2 = xx2.chunk(2, dim=-1)
            # print(x1.shape)
            out_avg, out_max = self.forward_corev2_multimodal(x1=x1.transpose(1, 2),x2=x2.transpose(1, 2))
            out_avg = out_avg * F.silu(z1)
            out_max = out_max * F.silu(z2)
            # print(out_avg.shape)
            return self.linearout(out_avg+out_max)


    class Channel_SS2D(nn.Module):
        """
        最终的、基于Mamba的通道注意力模块 (Channel-SS2D)。
        输入: (B, C, H, W) 特征图
        输出: (B, C, H, W) 经过通道注意力加权的特征图
        """
        ...
        def __init__(
            ...
            self.ssm_attn = MambaChannelAttention(
                d_model=dmodel,
                ssm_ratio=ssm_ratio,
                d_state=d_state,
                **kwargs
            )
            self.up = nn.Linear(1,self.dmodel)
            self.up1 = nn.Linear(1,self.dmodel)
            # attention_in_dim = self.ssm_attn.d_inner
            # self.attention = nn.Sequential(
            #     nn.Conv2d(channels, channels//16,1,bias=False),
            #     nn.SiLU(inplace=True),
            #     nn.Conv2d(channels//16, channels, 1, bias=False)
            # )
            # self.linaer1 = nn.Sequential(nn.Conv2d(channels,channels//16,1,bias=False),
            #                              nn.ReLU(inplace=True),
            #                              nn.Conv2d(channels//16,channels,1,bias=False)
            #                              )

            self.linaer2 = nn.Linear(2*dmodel,1)
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            b, c, _, _ = x.shape
            avg_feat = self.avg_pool(x)
            max_feat = self.max_pool(x)
            avg_feat1 = avg_feat.view(b, c).unsqueeze(2)
            max_feat1 = max_feat.view(b, c).unsqueeze(2)
            fused = self.ssm_attn(self.up(avg_feat1), self.up1(max_feat1))
            # fused = self.linaer1(modal_avg.view(b,-1,1,1)+avg_feat)+self.linaer1(modal_max.view(b,-1,1,1)+max_feat)
            fused = fused.view(b,-1,1,1)

            att_weights = F.sigmoid(fused)
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        # print(ys.shape)
        y_rgb, y_e = CrossMerge_multimodal.apply(ys,H,W)
    ...
    def cross_selective_scan_multimodal_1d(
        ...
        x_fuse = CrossScan1D_multimodal.apply(x1, x2)  # B, 1, H -> B, 2, 1, 2 * H
        # print(x_fuse.shape)
        # print(x_proj_weight.shape)
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", x_fuse, x_proj_weight)
        if x_proj_bias is not None:
        ...
        # print(ys.shape)
        y_rgb, y_e = CrossMerge1D_multimodal.apply(ys, H)

        y_rgb = y_rgb.transpose(dim0=1, dim1=2).contiguous().view(B, H, -1)
        y_e = y_e.transpose(dim0=1, dim1=2).contiguous().view(B, H, -1)
        y_rgb = out_norm1(y_rgb).to(x1.dtype)
        y_e = out_norm2(y_e).to(x2.dtype)
...
class ChannelSSM(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.expand = ssm_ratio
        self.d_inner = int(ssm_ratio * in_dim)
        self.dt_rank = math.ceil(in_dim / 16) if dt_rank == "auto" else dt_rank

        # 双分支投影
        self.proj_avg = nn.Sequential(
            nn.Linear(in_dim, self.d_inner),
            nn.SiLU()
        )
        self.proj_max = nn.Sequential(
            nn.Linear(in_dim, self.d_inner),
            nn.SiLU()
        )
        ...
        # SSM参数
        self.K = 2  # 扫描方向数
        self._build_ssm_params(self.d_inner, d_state)

    def _build_ssm_params(self, d_inner, d_state):
        """构建SSM参数矩阵"""
        # x投影参数
        self.x_proj_weight = nn.Parameter(torch.randn(
            self.K, d_inner, (self.dt_rank + d_state * 2)
        ))

        # delta参数
        self.dt_proj_weight = nn.Parameter(torch.randn(
            self.K, d_inner, self.dt_rank
        ))
        self.dt_proj_bias = nn.Parameter(torch.randn(self.K, d_inner))

        # 状态矩阵
        self.A_log = nn.Parameter(torch.randn(
            self.K * d_inner, d_state
        ))
        self.D = nn.Parameter(torch.ones(self.K * d_inner))

        # 输出归一化
        self.norm_avg = nn.LayerNorm(d_inner)
        self.norm_max = nn.LayerNorm(d_inner)

    def forward(self, avg: torch.Tensor, max: torch.Tensor):
        """双模态扫描流程"""
        # 投影到高维空间
        avg = self.proj_avg(avg)  # (B, d_inner)
        max = self.proj_max(max)  # (B, d_inner)

        # 执行交叉扫描
        out_avg, out_max = cross_selective_scan_1d_multimodal(
            x1=max,  # 对应原x1参数
            x2=avg,  # 对应原x2参数
            x_proj_weight=self.x_proj_weight,
            x_proj_bias=None,
            dt_projs_weight=self.dt_proj_weight,
            dt_projs_bias=self.dt_proj_bias,
            A_logs=self.A_log,
            Ds=self.D,
            out_norm1=self.norm_avg,
            out_norm2=self.norm_max
        )

        return out_avg, out_max
...
class SSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, L, d = x.shape
        x = x.permute(0, 2, 1)
        # print(rearrange(x, "b d l -> (b l) d").shape)
...
class cu_mamba_s(nn.Module):
    def __init__(
        ...
        # x proj; dt proj ============================
        self.linear1 = nn.Linear(self.d_inner,2*self.d_inner,bias=False)
        self.linear2 = nn.Linear(self.d_inner, 2*self.d_inner, bias=False)
        self.linear3 = nn.Linear(2*self.d_inner, self.d_inner, bias=False)
        self.conv1 = nn.Conv1d(2*self.d_inner,2*self.d_inner,kernel_size=3,padding=1)
        self.silu = nn.SiLU()
        self.x_proj_1 = nn.Linear(2*self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)
        self.x_proj_2 = nn.Linear(2*self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)

        self.dt_proj_1 = self.dt_init(self.dt_rank, 2*self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                                    **factory_kwargs)
        self.dt_proj_2 = self.dt_init(self.dt_rank, 2*self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                                    **factory_kwargs)

        # A, D =======================================
        self.A_log_1 = self.A_log_init(self.d_state, 2*self.d_inner)  # (D, N)
        self.A_log_2 = self.A_log_init(self.d_state, 2*self.d_inner)  # (D)
        self.D_1 = self.D_init(2*self.d_inner)  # (D)
        self.D_2 = self.D_init(2*self.d_inner)  # (D)

        # out norm ===================================
        self.out_norm_1 = nn.LayerNorm(2*self.d_inner)
        self.out_norm_2 = nn.LayerNorm(2*self.d_inner)
    ...
    def forward(self, x_rgb: torch.Tensor):
        ...
        x_rgb = self.linear1(x_rgb)
        x_rgb = self.silu(self.conv1(x_rgb.permute(0, 2, 1)))
        ...
        # assert out_y.dtype == torch.float
        y_rgb = rearrange(y_rgb, "b d l -> b l d")
        y_rgb = self.out_norm_1(y_rgb) * x


        return self.linear3(y_rgb)


class cu_mamba_c(nn.Module):
    def __init__(
            self,
            # basic dims ===========
            d_model=96,
        ...
        # x proj; dt proj ============================
        self.linear1 = nn.Linear(self.d_inner,self.d_inner,bias=False)
        self.linear2 = nn.Linear(self.d_inner, self.d_inner, bias=False)
        self.linear3 = nn.Linear(self.d_inner, self.d_inner, bias=False)
        self.conv1 = nn.Conv1d(self.d_inner,self.d_inner,kernel_size=3,padding=1)
        self.silu = nn.SiLU()
        self.x_proj_1 = nn.Linear(self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)
        self.x_proj_2 = nn.Linear(self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)
...
class ConcatMambaFusionBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm = norm_layer(hidden_dim)
        # self.norm1 = norm_layer(hidden_dim)
        # self.norm11 = norm_layer(hidden_dim)
        self.n = norm_layer(hidden_dim)
        self.mlp_branch = mlp_ratio > 0
        if self.mlp_branch:
            self.norm2 = norm_layer(hidden_dim)
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
...
class StateFusion(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # 使用多尺度空洞卷积（保持论文设计）
        self.conv_d1 = nn.Conv2d(dim, dim, 3, padding=1, dilation=1, groups=dim)
        self.conv_d3 = nn.Conv2d(dim, dim, 3, padding=3, dilation=3, groups=dim)
        self.conv_d5 = nn.Conv2d(dim, dim, 3, padding=5, dilation=5, groups=dim)
...
class StructureAwareSSM(nn.Module):
    def __init__(
        ...
        # 使用VMamba的CrossScan机制
        self.scan = CrossScan_multimodal()
        self.merge = CrossMerge_multimodal()

        # 修改后的selective_scan调用
        self.selective_scan = partial(
            self.cross_selective_scan,
            nrows=1  # 单次扫描
        )
    @staticmethod
    ...
    @staticmethod
    def cross_selective_scan(self, x, delta, A, B, C, D, delta_bias, delta_softplus, nrows=1):
        # 使用VMamba的扫描逻辑
        B, C, H, W = x.shape
        L = H * W
        ...
        # 结构感知状态融合（SASF）
        h = self.state_fusion(y)  # 保持原有的dilated conv融合
