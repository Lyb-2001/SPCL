...
def evaluate(logdir, save_predict=False, options=['val', 'test', 'test_day', 'test_night'], prefix=''):
    ...
    model.load_state_dict(torch.load("/media/map/fba69cc5-db71-46d1-9e6d-702c6d5a85f4/1SUS/run/839(SUS-Net1vmamba)/model.pth", map_location=device), strict=False)
    ...
    save_path = os.path.join('./result/', 'SPCNet-FE*')
        ...
        with torch.no_grad():
            ...
            for i, sample in tqdm(enumerate(test_loader), total=len(test_loader)):
                ...
                if cfg['inputs'] == 'rgb':
                    ...
                    # predict = (1-out[2])*out[0]+out[2]*out[1]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--logdir", type=str, default="/media/map/fba69cc5-db71-46d1-9e6d-702c6d5a85f4/1SUS/run/839(SUS-Net1vmamba)/SUS_RGB.json")
